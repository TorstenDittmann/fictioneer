...
function build_context_string(context: unknown): string {
  ...
  if (ctx.genre && typeof ctx.genre === "string")
    parts.push(`Genre: ${ctx.genre}`);
  if (ctx.tone && typeof ctx.tone === "string") parts.push(`Tone: ${ctx.tone}`);
  if (Array.isArray(ctx.character_names)) {
    const names = ctx.character_names.filter(
...
app.post("/api/continue", async (c) => {
  try {
    ...
    const word_count =
      typeof body?.word_count === "number" ? body.word_count : 100;
    ...
    const ctx =
      context && typeof context === "object" && context !== null
        ...
        : {};
    const recent_text =
      ctx.recent_text && typeof ctx.recent_text === "string"
        ? ctx.recent_text
        : "";
