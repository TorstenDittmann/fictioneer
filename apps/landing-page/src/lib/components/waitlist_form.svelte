...
<section id="newsletter" class="relative overflow-hidden px-4 py-20 sm:py-32">
	...
	<div class="aurora-blob top-8 left-0 h-56 w-56 bg-[rgba(124,140,255,0.2)]"></div>
	<div class="aurora-blob right-6 bottom-0 h-64 w-64 bg-[rgba(245,168,255,0.2)]"></div>

	<div class="relative mx-auto max-w-4xl">
		{#if is_success}
			<div
				class="glass-strong relative overflow-hidden rounded-[28px] border border-white/60 p-10 text-center shadow-2xl sm:p-12"
			...
			>
				...
				<div class="relative">
					<div class="mb-6 flex justify-center sm:mb-8">
						<div class="relative">
							<div
								class="relative flex h-20 w-20 items-center justify-center rounded-full bg-linear-to-br from-paper-accent via-paper-accent-light to-paper-accent-pink shadow-lg sm:h-24 sm:w-24"
							>
								<svg
					...
					<div
						class="glass mx-auto mb-6 max-w-lg rounded-2xl border border-white/60 p-6 shadow-inner shadow-white/60 sm:mb-8"
					>
						<div class="space-y-4">
		...
		{:else}
			...
			>
				<div
					class="absolute inset-0 bg-linear-to-br from-paper-accent/12 via-white/65 to-paper-accent-light/14"
				></div>
				<div class="relative">
					...
					<h2
						class="mb-4 font-serif text-3xl text-paper-text sm:mb-6 sm:text-4xl md:text-6xl"
					...
					<p
						class="mx-auto mb-10 max-w-3xl text-lg text-paper-text-light sm:mb-12 sm:text-xl md:text-2xl"
						class:animate-fade-in-up={mounted}
						class:opacity-0={!mounted}
					...
					>
						<div class="group relative">
							<div
								class="glass relative flex flex-col gap-3 rounded-2xl border border-white/60 p-2 shadow-inner shadow-white/60 sm:flex-row"
							>
								<input
									type="email"
									bind:value={email}
									...
									class="flex-1 rounded-xl border border-white/60 bg-white/80 px-4 py-3 text-paper-text placeholder-paper-text-muted transition-all outline-none focus:border-paper-accent focus:ring-2 focus:ring-paper-accent/20 disabled:opacity-50 sm:px-6 sm:py-4"
								/>

								<button
									type="submit"
									disabled={is_submitting || !email}
									class="btn-primary transition-smooth px-6 py-3 whitespace-nowrap shadow-lg disabled:cursor-not-allowed disabled:opacity-50 sm:px-8 sm:py-4"
								>
									{#if is_submitting}
						...
						{#if error_message}
							<div
								class="animate-fade-in mt-3 rounded-xl border border-red-500/20 bg-red-500/10 p-4 sm:mt-4"
							>
					...
					<div
						class="mx-auto mt-12 grid max-w-3xl grid-cols-1 gap-4 sm:mt-16 sm:grid-cols-3 sm:gap-6"
						class:animate-fade-in-up={mounted}
						class:opacity-0={!mounted}
						style:animation-delay="0.4s"
					>
						<div
							class="glass hover-lift rounded-xl border border-white/60 p-4 text-center shadow-sm sm:p-6"
						>
							<svg
								class="mx-auto mb-3 h-7 w-7 text-paper-accent sm:h-9 sm:w-9"
								fill="none"
								stroke="currentColor"
								viewBox="0 0 24 24"
							>
